...
common subexpression elimination (the call to \texttt{Combine} in
...
textbook does not need a common subexpression elimination step, because it is
always covering the same graph. On the other hand, the e-graph models all
possile restructuring of the logic according to Boolean algebra. Hence, the
e-graph extractor presents many `best' solutions. As a result,
Alg.~\ref{alg:dynprog} is quick but produces suboptimal results. I attempted to
...
benchmarks. I spend the large majority of the last two weeks making ILP
