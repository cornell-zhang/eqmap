...
use std::{cmp, process};
use sv_parser::{
    parse_sv, unwrap_node, Define, DefineText, Error, Identifier, ListOfPortConnections, Locate,
    NamedPortConnection, NodeEvent, PortIdentifier, RefNode, SyntaxTree,
};
...
fn run_opt(opt: &Opt) -> i32 {
    ...
    for define in &opt.defines {
        ...
        let text = if let Some(x) = define.next() {
            ...
            Some(DefineText::new(x.to_string(), None))
        } else {
            None
        };
...
fn show_macro_defs(defines: &HashMap<String, Option<Define>>) {
...
fn get_rec_expr_from_tree(syntax_tree: &SyntaxTree) -> String {
    // &SyntaxTree is iterable
    for node in syntax_tree {
        // The type of each node is RefNode
        let nstr = match node {
            ...
            RefNode::ModuleInstantiation(x) => {
                ...
                format!("            inst_name: {}", escape_str(id))
            }
            _ => "\n".to_string(),
        };
        return "hi".to_string();
...
fn get_identifiers(node: RefNode) -> Vec<Locate> {
    // unwrap_node! can take multiple types
    match unwrap_node!(
        node,
        SimpleIdentifier,
        ...
        Some(RefNode::PortIdentifier(x)) => match &x.nodes.0 {
            Identifier::SimpleIdentifier(x) => vec![x.nodes.0],
            Identifier::EscapedIdentifier(x) => vec![x.nodes.0],
        },
        Some(RefNode::ListOfPortConnections(x)) => {
            let mut ret = vec![];
            let nodes = match x {
                ListOfPortConnections::Named(x) => x.nodes.0.clone(),
                _ => panic!("ListOfPortConnections not Named"),
            };
            for node in nodes.contents() {
                let id = match node {
                    NamedPortConnection::Identifier(x) => x.nodes.2.clone(),
                    NamedPortConnection::Asterisk(_) => panic!("Asterisk not supported"),
                };
                match id.nodes.0 {
                    Identifier::SimpleIdentifier(x) => ret.push(x.nodes.0),
                    Identifier::EscapedIdentifier(x) => ret.push(x.nodes.0),
                }
            }
            ret
        }
...
mod tests {
    ...
    fn test_def_test() {
        let opt = Opt {
            ...
            defines: vec![
                String::from("MODULE_NAME=module_name_from_define"),
