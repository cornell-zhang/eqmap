...
impl ExtractStrat {
    ...
    /// Create an extraction strategy from a set of gates you want to extract.
    pub fn from_gate_set(list: &str) -> Result<Self, String> {
        ...
        for gate in &gates {
            if !Self::WHITELIST.contains(&gate.as_str()) {
                return Err(format!("Gate {} is not in the whitelist", gate));
            }
        }
    ...
    A: Analysis<LutLang>,
{
    ...
    /// Extract only select gates from `list`
        ...
        A: Analysis<LutLang> + std::default::Default,
    {
        match &self.extract_strat {
            ...
            ExtractStrat::Disassemble(set) => {
                self.greedy_extract_with(GateCostFn::new(set.clone()))
            }
            #[cfg(feature = "exactness")]
